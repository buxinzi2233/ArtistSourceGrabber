...
>
> [!致谢]
> 感谢https://linux.do/社区提供的交流平台与支持。特此致谢。
...
- Google Chrome。X 与 Pixiv 的推荐登录方式依赖专用 Chrome Profile。
...
1. 在“混合来源”中勾选需要执行的来源。
2. 输入 Danbooru 画师名、别名，或已知的 X 主页 URL。
3. 点击模糊搜索，并人工确认候选画师是否为同一个人。
4. 检查自动带出的 X handle、Pixiv 数字用户 ID 和各来源标签。
5. 对 X 或 Pixiv，首次使用时打开专用登录窗口并自行完成登录。
6. 选择下载数量、内容分级、标签格式和打标器。
7. 先使用“测试来源连接”和“测试打标器”。
8. 确认结果正常后开始下载。
9. 在作品工作区查看原图、来源状态和每张图片的打标详情。
...
```text
%LOCALAPPDATA%\DanbooruGrabber\XBrowserProfile
%LOCALAPPDATA%\DanbooruGrabber\PixivBrowserProfile
```
